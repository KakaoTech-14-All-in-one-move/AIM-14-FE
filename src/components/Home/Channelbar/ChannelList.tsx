...
const ChannelList: React.FC<{ type: ChannelType; icon: React.ElementType }> = ({
  type,
  icon: Icon,
}) => {
  const {
    ...
    <div className="mt-5">
      ...
      >
        <div className="flex items-center">
          <ChevronDown
            size={12}
            className={`mr-1 transform transition-transform ${openSections[type] ? '' : '-rotate-90'}`}
          />
      ...
      {openSections[type] &&
        channels[type].map((channel: string) => (
