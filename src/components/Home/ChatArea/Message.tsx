...
const Message: React.FC<MessageProps> = React.memo(({ author, contents, showHeader, isCurrentUser, userColor }) => {
        ...
        <div className="flex mb-4">
            {showHeader && (
                <div className="flex-shrink-0 mr-3 self-start pt-1">
                    {isCurrentUser ? (
                        <img src={profileImage} alt={author} className="w-10 h-10 rounded-full object-cover" />
                    ) : (
                        <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold" style={{ backgroundColor: userColor }}>
                            {initials}
                        </div>
                    )}
                </div>
